...
class SemiSupervisedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, base_dataset='cifar10', take_amount=None, take_amount_seed=13, aux_data_filename=None, 
        ...
        if self.train:
            ...
            if aux_data_filename is not None:
                ...
                if os.path.splitext(aux_path)[1] == '.pickle':
                ...
                elif os.path.splitext(aux_path)[1] == '.h5':
                    # Open h5 file but don't load data into memory
                    self.h5_file = h5py.File(aux_path, 'r')
                    
                    # Get references to datasets without loading them
                    self.aux_data = self.h5_file['image']  
                    self.aux_targets = self.h5_file['label']
                    
                    print(f"H5 dataset shape: {self.aux_data.shape}")
                    
                    # Store original data length before adding aux data
                    ...
                    if aux_take_amount is not None:
                        rng_state = np.random.get_state()
                        np.random.seed(take_amount_seed)
                        self.aux_indices = np.random.choice(len(self.aux_data), 
                                                        aux_take_amount, 
                                                        replace=False)
                        np.random.set_state(rng_state)
                    else:
                        self.aux_indices = np.arange(len(self.aux_data))
                    
                    # Modify dataset properties to handle on-demand loading
                    # Instead of concatenating, we'll override __getitem__ method
                    
                    # Update targets list with placeholder values
                    if not add_aux_labels:
                        self.targets.extend([-1] * len(self.aux_indices))
                    else:
                        # For labels, we might need to load them since they're smaller
                        aux_targets_subset = self.aux_targets[self.aux_indices]
                        self.targets.extend(aux_targets_subset)
                    
                    # Update unsup_indices to include the new indices
                    self.unsup_indices.extend(range(orig_len, orig_len + len(self.aux_indices)))
                    
                    # Flag to indicate we're using h5 data
                    self.using_h5 = True
                    ...
                    #if os.path.splitext(aux_path)[1] != '.h5':
    ...
    def __getitem__(self, item):
        # Check if this item is from the h5 file
        if hasattr(self, 'using_h5') and self.using_h5 and item >= len(self.data):
            # Calculate the index in the aux data
            h5_idx = self.aux_indices[item - len(self.data)]
            
            # Load just this single item from the h5 file
            img = self.aux_data[h5_idx]
            target = self.targets[item]  # We've already added these to targets
            
            img = Image.fromarray(img)

            # Apply any transformations your dataset normally does
