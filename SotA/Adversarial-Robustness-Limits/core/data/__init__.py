...
def load_data(data_dir, batch_size=256, batch_size_test=256, num_workers=4, use_augmentation='base', use_consistency=False, shuffle_train=True, 
            ...
            distributed_env=None, unfix_N_batches_per_epoch=False, better_sampler=False,
            EDM_50_amount=None, args=None):
    """
    ...
    if validation:
        ...
        train_dataset, test_dataset, val_dataset = load_dataset_fn(data_dir=data_dir, use_augmentation=use_augmentation, use_consistency=use_consistency,
                                                                   aux_data_filename=aux_data_filename, validation=True, EDM_50_amount=EDM_50_amount, env=distributed_env, start_epoch=args.start_epoch)
    else:
    ...
    if dataset in SEMISUP_DATASETS:
        if validation:
            train_dataloader, test_dataloader, val_dataloader = get_semisup_dataloaders(
                train_dataset, test_dataset, val_dataset, batch_size, batch_size_test, num_workers, unsup_fraction, distributed=distributed_env, 
                unfix_N_batches_per_epoch=unfix_N_batches_per_epoch, better_sampler=better_sampler, args=args
            )
        else:
            train_dataloader, test_dataloader = get_semisup_dataloaders(
                train_dataset, test_dataset, None, batch_size, batch_size_test, num_workers, unsup_fraction, distributed=distributed_env, 
                unfix_N_batches_per_epoch=unfix_N_batches_per_epoch, better_sampler=better_sampler, args=args
            )
    else:
        assert not distributed_env, 'Distributed is turned on, but we have not set up distributed dataloaders for this dataset'
