...
def filter_trades_loss(model, x_natural, y, optimizer, step_size=0.003, epsilon=0.031, perturb_steps=10, beta=1.0, 
                attack='linf-pgd', label_smoothing=0.1, use_cutmix=False, use_consistency=False, cons_lambda=0.0, cons_tem=0.0,
                device=None, train_prepro = None):
    """
    TRADES training (Zhang et al, 2019).
    """
    ...
    lower = torch.clamp(x_natural - epsilon, 0, 1)
    upper = torch.clamp(x_natural + epsilon, 0, 1)
    ...
    if attack == 'linf-pgd':
        preprocessor = train_prepro
        classifier = model.module[1]
        for _ in range(perturb_steps):
            x_adv = preprocessor(x_adv)
            x_adv.requires_grad_()
            with torch.enable_grad():
                loss_kl = criterion_kl(F.log_softmax(classifier(x_adv), dim=1), p_natural)
            grad = torch.autograd.grad(loss_kl, [x_adv])[0]
            x_adv = x_adv.detach() + step_size * torch.sign(grad.detach())
        ...
        raise ValueError(f'Attack={attack} not supported for Filter TRADES training!')
    model.train()
    track_bn_stats(model, True)
...
def trades_loss_LSE(model, x_natural, y, optimizer, step_size=0.003, epsilon=0.031, perturb_steps=10, beta=1.0, 
                attack='linf-pgd', label_smoothing=0.1, clip_value=0, use_cutmix=False, num_classes=10, device=None):
    """
    SCORE training (Ours).
    """
    # criterion_ce = SmoothCrossEntropyLoss(reduction='mean', smoothing=label_smoothing)
