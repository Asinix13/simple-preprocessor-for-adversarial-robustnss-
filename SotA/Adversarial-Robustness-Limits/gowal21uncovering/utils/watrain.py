...
from .trades import trades_loss, trades_loss_LSE, filter_trades_loss
from .cutmix import cutmix


from core.utils.distributed import all_reduce_mean_and_reweight
from core.models import Bil_layer
...
class WATrainer(Trainer):
    ...
    def __init__(self, info, args, distributed=None):
        ...
        self.params = args
        self.model = create_model(args.model, args.normalize, info, self.device, args,
        ...
        if self.params.gvar != 0.:
            self.train_preprocessor = Bil_layer(bil = self.params.bil, sigma_color =.5, color2 = 0.05)
            

        super(WATrainer, self).__init__(info, args, self.device)
            ...
            if self.params.consistency:
                ...
                if adversarial:
                    if self.params.beta is not None and self.params.mart:
                        ...
                        if (self.params.filter and (update_iter%2) == 0):
                            loss, batch_metrics = self.filter_trades_loss(x, y, beta=self.params.beta,
                                                                           train_prepro=self.train_preprocessor)     
                        else: 
    ...
    def filter_trades_loss(self, x, y, beta, train_prepro):
        """
        Filter TRADES training.
        """
        loss, batch_metrics = filter_trades_loss(self.model, x, y, self.optimizer, step_size=self.params.attack_step, 
                                          epsilon=self.params.attack_eps, perturb_steps=self.params.attack_iter, 
                                          beta=beta, attack=self.params.attack, label_smoothing=self.params.ls,
                                          use_cutmix=self.params.CutMix,
                                          device=self.device, train_prepro=train_prepro)
